...
from models.arch.default import UNet_SE
...
class RUNetModel(ERRNetBase):
    ...
    def print_network(self):
        print('--------------------- Model ---------------------')
        print('##################### NetU #####################')
        networks.print_network(self.net_u)
    ...
    def initialize(self, opt):
        ...
        if opt.hyper:
            self.vgg = self.vgg = losses.Vgg19(requires_grad=False).to(self.device)

        self.net_u = arch.__dict__[self.opt.inet](in_channels, 3).to(self.device)
        # using default initialization as EDSR
        networks.init_weights(self.net_u, init_type=opt.init_type)
        self.edge_map = EdgeMap(scale=1).to(self.device)

        if self.isTrain:
            # define loss functions
            self.loss_dic = losses.init_unet_loss(opt, self.Tensor)
            vggloss = losses.ContentLoss()
            vggloss.initialize(losses.VGGLoss(self.vgg, weights=[1.0/2.6], indices=[2], normalize=False))
            self.loss_dic['r_vgg'] = vggloss

            # initialize optimizers
            self.optimizer_G = torch.optim.Adam(self.net_u.parameters(),
                lr=opt.lr, betas=(0.9, 0.999), weight_decay=opt.wd)
    ...
    def backword_G(self):
        ...
        self.loss_icnn_pixel = self.loss_dic['r_pixel'].get_loss(
            self.output_i, self.target_r)

        self.loss_icnn_vgg = self.loss_dic['r_vgg'].get_loss(
            self.output_i, self.target_r)
    ...
    def forward(self):
        # without edge
        input_i = self.input
        input_t = self.target_t
        input_r = input_i - input_t
        # print(torch.cat([input_i, input_r], dim=1).shape)
        output_i = self.net_u(torch.cat([input_i, input_r], dim=1))
    ...
    def get_current_visuals(self):
        ...
        ret_visuals['input'] = tensor2im(self.input).astype(np.uint8)
        ret_visuals['output_i'] = tensor2im(self.output_i).astype(np.uint8)
        ret_visuals['target'] = tensor2im(self.target_r).astype(np.uint8)
        ret_visuals['residual'] = tensor2im((self.input - self.output_i)).astype(np.uint8)
