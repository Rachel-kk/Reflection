...
class CSFlow:
    ...
    def create_using_dotP(I_features, T_features, sigma=float(0.5), b=float(1.0)):
        ...
        for i in range(N):
            ...
            patches_PC11_i = cs_flow.patch_decomposition(
                T_features_i)  # 1CHW --> PC11, with P=H*W (C_out x C_in x H x W)
    ...
    def center_by_T(self, T_features, I_features):
        ...
        axes = [0, 1, 2]
        self.meanT = T_features.mean(TensorAxis.N, keepdim=True).mean(TensorAxis.H, keepdim=True).mean(TensorAxis.W,
                                                                                                       keepdim=True)
